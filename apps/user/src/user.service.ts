...
export class UserService {
  ...
  async adminUpdateLevel(data: { id: number; name?: string; minPoints?: number; maxPoints?: number; order?: number; description?: string; badgeUrl?: string; isActive?: boolean }) {
    ...
    Object.assign(level, rest);
    await this.levelRepo.save(level);
    return { code: 200, data: level };
  }

  async adminDeleteLevel(id: number) {
    const level = await this.levelRepo.findOne({ where: { id } });
    if (!level) return { code: 404, data: null, message: 'Level not found' };
    await this.levelRepo.remove(level);
    return { code: 200, data: null, message: 'Level deleted' };
  }
  // LEVELS
  ...
  async adminSetPersonalId(data: { userId: string; personalId: string }) {
    ...
    return {
      code: 200,
      data: { personalId: data.personalId },
      message: 'Personal ID updated',
    };
  }
  ...
  async adminGetUserWagering(userId: string) {
    ...
    if (!stats)
      return { code: 404, data: null, message: 'No wagering record found' };
    return { code: 200, data: stats };
  }

  // ADMIN: GET ALL USERS WAGERING (paginated)
  async adminGetAllWagering(filters?: {
    page?: number;
    limit?: number;
    search?: string;
  }) {
