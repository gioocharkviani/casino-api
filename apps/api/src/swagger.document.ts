...
export const swaggerDocument: Omit<OpenAPIObject, 'paths'> & {
  ...
  components: {
    ...
    schemas: {
      ...
      // â”€â”€ WALLET â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      DepositRequest: {
        type: 'object',
        required: ['amount'],
        properties: {
      ...
      // â”€â”€ PROMOTIONS (user) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // â”€â”€ ADMIN PROMOTIONS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // â”€â”€ GENERIC RESPONSES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      SuccessResponse: {
        type: 'object',
        properties: {
  ...
  paths: {
    ...
    '/api/user/sign-in': {
      post: {
        tags: ['User'],
        summary: 'Sign in â€” returns Bearer token',
    ...
    '/api/user/sign-out': {
      post: {
        tags: ['User'],
        summary: 'Sign out â€” invalidates token',
        security: [{ bearerAuth: [] }],
    ...
    '/api/admin/sign-in': {
      post: {
        tags: ['Admin / Auth'],
        summary: 'Admin sign in â€” returns Bearer token',
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // ADMIN â€” ADMIN MANAGEMENT  (super_admin only)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    '/api/admin/admins': {
      get: {
    ...
    '/api/admin/users/{userId}/block': {
      put: {
        tags: ['Admin / Users'],
        summary: 'Block user â€” invalidates all sessions  [admin, super_admin]',
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'userId', in: 'path', required: true, schema: { type: 'string' } }],
    ...
    '/api/admin/games/revolver-raw': {
      get: {
        tags: ['Admin / Games'],
        summary: 'Preview raw game list from Revolver (no DB writes)  [any admin]',
        description:
          'Calls Revolver\'s getGamesList API directly and returns the raw response so you can inspect exactly what the provider sent back, without running an actual sync.',
        security: [{ bearerAuth: [] }],
        responses: {
          200: { description: '{ status, gameCount, raw: <full Revolver response> }' },
          401: { description: 'Missing/invalid admin session' },
          502: { description: 'Revolver API request failed' },
        },
      },
    },
    '/api/admin/games/nuxgame-raw': {
      get: {
        tags: ['Admin / Games'],
        summary: 'Preview raw game list from NuxGame (no DB writes)  [any admin]',
        description:
          'Calls NuxGame\'s /providersList and /gameList APIs directly and returns the raw arrays so you can inspect exactly what the provider sent back (e.g. check whether image fields are actually populated), without running an actual sync.',
        security: [{ bearerAuth: [] }],
        responses: {
          200: { description: '{ providerCount, gameCount, providers: [...], games: [...] }' },
          401: { description: 'Missing/invalid admin session' },
